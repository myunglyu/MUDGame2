...
public class GameService : IGameService
{
    ...
    public async Task<string> ProcessCommand(Character character, string command)
    {
        ...
        var _command = command.ToLower().Split(" ");
        if (_command[0] == "/attack"){
            var monster = await _context.Monsters.FirstOrDefaultAsync(x => x.Name == _command[1]);
            if (monster != null){
                var result = await BattleAsync(_character, monster);
                if (result){
                    return $"{_character.Name} defeated the monster!";
                }
                else{
                    return $"{_character.Name} were defeated by the monster!";
                }
            }
            else{
